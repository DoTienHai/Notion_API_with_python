...
def update_luy_ke_theo_ngay(location):
    ...
    # database_id = dataBaseDict["Lũy kế ngày HỆ THỐNG"]
    # json_file_path = os.path.join(notion_data_folder, 'Lũy kế ngày HỆ THỐNG.json')
    # if (location in location_list):
    database_id = dataBaseDict[f"Lũy kế ngày {location}"]
    json_file_path = os.path.join(notion_data_folder, f'Lũy kế ngày {location}.json')
...
def update_luy_ke_theo_thang(location):
    ...
    # Đường dẫn tới tệp JSON của bạn
    # API key và database ID của bạn
    database_id = dataBaseDict["Lũy kế tháng HỆ THỐNG"]
    json_file_path = os.path.join(notion_data_folder, 'Lũy kế tháng HỆ THỐNG.json')
    if (location in location_list):
        database_id = dataBaseDict[f"Lũy kế tháng HỆ THỐNG"]
        json_file_path = os.path.join(notion_data_folder, f'Lũy kế tháng {location}.json')
